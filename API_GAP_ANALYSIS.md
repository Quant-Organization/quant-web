...
> 작성일: 2026-03-04
> FastAPI 서버: `VITE_FASTAPI_URL` (주식 시뮬레이터)
> Spring Boot 서버: `VITE_SPRING_API_URL` (게임 백엔드)
...
### ~~1.4 보관시설 관리 (Storage)~~ ✅ 해결됨

| 항목 | 내용 |
|------|------|
| **경로** | 각 자산 페이지 내 탭 (`/asset/vehicles`, `/asset/jet`, `/asset/yacht`) |
| **상태** | ~~로컬 Svelte Store + localStorage~~ → **제거됨** |

**해결 내용:**
- 서버에 보관시설 관련 API가 없으므로 localStorage 기반 보관시설(차고/격납고/마리나) 기능 전면 제거
- 자산 페이지에서 "보관시설 관리" 탭 제거, 서버 API 기반 컬렉션/마켓 2탭 구조로 변경
- `asset.ts` 스토어를 서버 기반 잔고(`getSpringAccount`)로 전환
- 보관시설 없이도 차량/전용기/요트 구매 가능 (서버가 유효성 검증 담당)
...
**해결 내용:**
- `special` — 서버 엔드포인트 미존재, 사이드바에서 제거
- `market` — `/asset/vehicles`로 리다이렉트 (기존 유지)
- `storage` — `/asset/vehicles`로 리다이렉트 (기존 유지), 보관시설 구매 하위 라우트 제거
