...
export function getCryptoHistory(cryptoId: string, limit = 500, interval = 5) {
	return fetchFastAPI<CryptoCandle[]>(`/api/crypto/history/${cryptoId}?limit=${limit}&interval=${interval}`);
}
...
export async function getCryptoHoldings() {
	const data = await fetchFastAPI<{ holdings: CryptoHolding[]; total_crypto_value: number }>('/api/crypto/holdings');
