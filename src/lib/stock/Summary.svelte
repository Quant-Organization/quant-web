...
<aside class="info-panel" class:expanded>
    <div class="summary-content">
        ...
        <div class="account-row">
            ...
            <span class="stat-value">{formatPrice(dashboard?.summary.cash ?? 0, currency)}</span>
        </div>
        <div class="account-row">
            <span class="stat-label">주식 평가액</span>
            <span class="stat-value">{formatPrice(dashboard?.summary.stock_value ?? 0, currency)}</span>
        </div>
        <div class="account-row">
            <span class="stat-label">총 자산</span>
            <span class="stat-value">{formatPrice(dashboard?.summary.total_assets ?? 0, currency)}</span>
        </div>
        <div class="account-row">
            <span class="stat-label">총 손익</span>
            <span class="stat-value" style="color: {(dashboard?.summary.total_profit_loss ?? 0) >= 0 ? '#16a34a' : '#dc2626'}">
                {(dashboard?.summary.total_profit_loss ?? 0) >= 0 ? '+' : ''}{formatPrice(dashboard?.summary.total_profit_loss ?? 0, currency)} ({(dashboard?.summary.total_profit_loss_pct ?? 0).toFixed(2)}%)
            </span>
        </div>
    </div>
