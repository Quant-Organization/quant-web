<script lang="ts">
    ...
    function getTierState(tierIdx: number): 'owned' | 'upgrade' | 'purchase' {
        if (!hangar) return 'purchase';
        const currentIdx = tierOrder.indexOf(hangar.tier);
        if (tierIdx <= currentIdx) return 'owned';
        return 'upgrade';
    }

    function handleTierPurchase(tier: StorageTier, name: string) {
        const success = purchaseStorage('hangar', tier);
...
<div class="page-container">
    <nav class="tab-bar">
        <button class="tab-btn" class:active={activeTab === 'collection'} onclick={() => activeTab = 'collection'}>내 전용기</button>
        <button class="tab-btn" class:active={activeTab === 'market'} onclick={() => activeTab = 'market'}>전용기 마켓</button>
        <button class="tab-btn" class:active={activeTab === 'storage'} onclick={() => activeTab = 'storage'}>격납고 관리</button>
    </nav>
    ...
    {#if activeTab === 'collection'}
        <header class="page-header">
            ...
            <div class="stats-cards">
                ...
                <div class="stat-card">
                    ...
                    <span class="stat-value">{formatCurrency(totalJetValue)}</span>
                </div>
            </div>
        </header>
        ...
        {:else if $ownedJets.length === 0}
            <div class="empty-state">
                ...
                <p class="empty-subtitle">마켓에서 첫 번째 전용기를 구매해보세요.</p>
            </div>
        {:else}
    ...
    {:else if activeTab === 'market'}
        {#if !hasStorage}
            <div class="empty-state">
                <div class="empty-icon"><span class="empty-icon-text">!</span></div>
                <h2 class="empty-title">전용기를 구매하려면 격납고가 필요합니다</h2>
                <p class="empty-subtitle">격납고를 먼저 구매하면 전용기를 구매할 수 있습니다.</p>
                <button class="btn-primary" onclick={() => activeTab = 'storage'}>격납고 구매하러 가기</button>
            </div>
        {:else}
            <header class="page-header">
            ...
            <main class="market-main-grid">
                ...
                <section class="m-right-section" in:fade={{delay: 200, duration: 300}} out:fade={{duration: 150}}>
                    <h3 class="section-title">프리미엄 항공사</h3>
                    <div class="m-filters">
    ...
    {:else}
        <section class="storage-section">
            <header class="page-header">
                <div class="header-text">
                    <h1>격납고 관리</h1>
                    <p>더 많은 전용기를 보관하기 위해 격납고를 업그레이드 하세요</p>
                </div>
                <div class="stats-cards">
                    <div class="stat-card">
                        <span class="stat-label">잔고</span>
                ...
                {#each tiers as tier, idx}
                    ...
                    <div class="card tier-card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="specs">
                                <div class="spec-row">
                                    <span class="label">전용기 보유 가능 대수</span>
                                    <span class="value">{tier.capacity}</span>
                                </div>
                                <div class="spec-row">
                                    <span class="label">이용 가능 등급</span>
                ...
                <div class="my-storage-section">
                    ...
                    <div class="card my-storage-card">
                        ...
                        <div class="card-body">
                            <h3>{hangar.name}</h3>
                            <div class="info-row">
                                <span class="info-item">보관 {hangar.currentCount} / {hangar.capacity}대</span>
                                <span class="info-item">{tiers.find(t => t.tier === hangar.tier)?.gradeText ?? ''}</span>
                            </div>
                            <div class="my-stats-row">
                                <div class="stat">
                                    <span class="stat-label">유지비/일</span>
                                    <span class="stat-value">{formatCurrency(hangar.maintenanceCost)}</span>
                                </div>
                                <div class="stat">
                                    <span class="stat-label">보관 가능</span>
                                    <span class="stat-value">{hangar.capacity}대</span>
                                </div>
                            </div>
                        </div>
                        <div class="card-actions">
                            <button onclick={() => activeTab = 'collection'}>전용기 보기</button>
