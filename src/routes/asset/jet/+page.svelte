<script lang="ts">
    ...
    import { getJets, getMyJets, purchaseJet, sellJet, getJetDetail, getAssetErrorMessage } from '$lib/api/asset';
    ...
    let ownedItems: JetItem[] = $state([]);
    let account: SpringAccount | null = $state(null);

    let loading = $state(true);
    let error = $state('');
    let selectedItem: JetItem | null = $state(null);

    async function selectItem(item: JetItem) {
        selectedItem = item;
        try { selectedItem = await getJetDetail(item.id); } catch { /* keep existing */ }
    }
    onMount(async () => {
        await loadAll();
    });
    ...
    async function loadAll() {
        try {
            ...
            if (Array.isArray(oRes)) {
                ownedItems = oRes;
            } else if (oRes && Array.isArray(oRes.jets)) {
                ownedItems = oRes.jets.map((v: any) => ({ ...v.jet, purchasedAt: v.purchasedAt, currentValue: v.currentValue, userItemId: v.id }));
            } else {
                ownedItems = [];
            }
            account = aRes;
    ...
    async function handlePurchase(item: JetItem) {
        try {
            await purchaseJet(item.id);
...
<div class="page-container">
    <header class="page-header">
        ...
        {#if account}
            <div class="stats-cards">
                ...
                <div class="stat-card">
                    <span class="stat-label">보유 전용기</span>
    ...
    <nav class="tab-bar">
        <button class="tab-btn" class:active={activeTab === 'owned'} onclick={() => { activeTab = 'owned'; selectedItem = null; }}>내 전용기</button>
        <button class="tab-btn" class:active={activeTab === 'market'} onclick={() => { activeTab = 'market'; selectedItem = null; }}>전용기 마켓</button>
    </nav>
                ...
                <div class="detail-info">
                    ...
                    <div class="spec-grid">
                        ...
                        <div class="spec-item">
                            ...
                            <strong>{selectedItem.rangeKm} km</strong>
                        </div>
                        <div class="spec-item">
                            <span class="spec-label">좌석수</span>
                            <strong>{selectedItem.seats}석</strong>
                        </div>
                        <div class="spec-item">
    ...
    {:else}
        {#if ownedItems.length === 0}
            <div class="empty-state">
                ...
                <p class="empty-subtitle">마켓에서 전용기를 구매해보세요.</p>
                <button class="btn-primary" onclick={() => (activeTab = 'market')}>전용기 마켓으로</button>
            </div>
