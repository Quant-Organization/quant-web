<script lang="ts">
    ...
    import { getYachts, getMyYachts, purchaseYacht, sellYacht, getYachtDetail, getAssetErrorMessage } from '$lib/api/asset';
    ...
    let ownedItems: YachtItem[] = $state([]);
    let account: SpringAccount | null = $state(null);

    let loading = $state(true);
    let error = $state('');
    let selectedItem: YachtItem | null = $state(null);

    async function selectItem(item: YachtItem) {
        selectedItem = item;
        try { selectedItem = await getYachtDetail(item.id); } catch { /* keep existing */ }
    }
    onMount(async () => {
        await loadAll();
    });
    ...
    async function loadAll() {
        try {
            ...
            if (Array.isArray(oRes)) {
                ownedItems = oRes;
            } else if (oRes && Array.isArray(oRes.yachts)) {
                ownedItems = oRes.yachts.map((v: any) => ({ ...v.yacht, purchasedAt: v.purchasedAt, currentValue: v.currentValue, userItemId: v.id }));
            } else {
                ownedItems = [];
            }
            account = aRes;
    ...
    async function handlePurchase(item: YachtItem) {
        try {
            await purchaseYacht(item.id);
...
<div class="page-container">
        ...
        {#if account}
            <div class="stats-cards">
                ...
                <div class="stat-card">
                    ...
                    <span class="stat-value">{ownedItems.length}척</span>
                </div>
            </div>
        {/if}
    </header>
    ...
    <nav class="tab-bar">
        <button class="tab-btn" class:active={activeTab === 'owned'} onclick={() => { activeTab = 'owned'; selectedItem = null; }}>내 요트</button>
        <button class="tab-btn" class:active={activeTab === 'market'} onclick={() => { activeTab = 'market'; selectedItem = null; }}>요트 마켓</button>
    </nav>
    ...
    {:else}
        {#if ownedItems.length === 0}
            <div class="empty-state">
                ...
                <p class="empty-subtitle">마켓에서 요트를 구매해보세요.</p>
                <button class="btn-primary" onclick={() => (activeTab = 'market')}>요트 마켓으로</button>
            </div>
