<script lang="ts">
    ...
    import { getRealEstates, getMyRealEstates, purchaseRealEstate, sellRealEstate, getRealEstateDetail, getAssetErrorMessage } from '$lib/api/asset';
    ...
    async function loadAll() {
        try {
            ...
            if (Array.isArray(oRes)) {
                ownedItems = oRes;
            } else if (oRes && Array.isArray(oRes.realEstates)) {
                ownedItems = oRes.realEstates.map((v: any) => ({ ...v.realEstate, purchasedAt: v.purchasedAt, currentValue: v.currentValue, userItemId: v.id }));
            } else {
                ownedItems = [];
            }
            account = aRes;
