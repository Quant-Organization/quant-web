<script lang="ts">
    ...
    import { getVehicles, getMyVehicles, purchaseVehicle, sellVehicle, getVehicleDetail, getAssetErrorMessage } from '$lib/api/asset';
    ...
    let activeTab: 'market' | 'owned' = $state('owned');

    let marketItems: VehicleItem[] = $state([]);
    let ownedItems: VehicleItem[] = $state([]);
    let account: SpringAccount | null = $state(null);

    let loading = $state(true);
    let error = $state('');
    let selectedItem: VehicleItem | null = $state(null);
    let selectedMarketVehicleId: number | null = $state(null);

    async function selectItem(item: VehicleItem) {
        selectedItem = item;
        if (activeTab === 'market') selectedMarketVehicleId = item.id;
        try { selectedItem = await getVehicleDetail(item.id); } catch { /* keep existing */ }
    }
    onMount(async () => {
        await loadAll();
    });
    ...
    async function loadAll() {
        try {
            ...
            if (Array.isArray(oRes)) {
                ownedItems = oRes;
            } else if (oRes && Array.isArray(oRes.vehicles)) {
                ownedItems = oRes.vehicles.map((v: any) => ({ ...v.vehicle, purchasedAt: v.purchasedAt, purchasePrice: v.purchasePrice, currentValue: v.currentValue, ownedDays: v.ownedDays, userItemId: v.id }));
            } else {
                ownedItems = [];
            }
            account = aRes;
    ...
    async function handlePurchase(item: VehicleItem) {
        const purchaseId = selectedMarketVehicleId ?? item.id;
        try {
            await purchaseVehicle(purchaseId);
            toast.success(`${item.name} 구매 완료!`);
            await loadAll();
            selectedItem = null;
            selectedMarketVehicleId = null;
        } catch (e: unknown) {
...
<div class="page-container">
        ...
        {#if account}
            <div class="stats-cards">
                ...
                <div class="stat-card">
                    ...
                    <span class="stat-value">{ownedItems.length}대</span>
                </div>
            </div>
        {/if}
    </header>
    ...
    <nav class="tab-bar">
        <button class="tab-btn" class:active={activeTab === 'owned'} onclick={() => { activeTab = 'owned'; selectedItem = null; selectedMarketVehicleId = null; }}>내 차량</button>
        <button class="tab-btn" class:active={activeTab === 'market'} onclick={() => { activeTab = 'market'; selectedItem = null; selectedMarketVehicleId = null; }}>차량 마켓</button>
    </nav>
    ...
    {:else if activeTab === 'market'}
        {#if selectedItem}
            <button class="back-btn" onclick={() => { selectedItem = null; selectedMarketVehicleId = null; }}>← 목록으로</button>
                ...
                <div class="detail-info">
                    ...
                    <div class="spec-grid">
                        ...
                        <div class="spec-item">
                            ...
                            <strong>{selectedItem.maxSpeed} km/h</strong>
                        </div>
                        <div class="spec-item">
                        ...
                        {#if selectedItem.color}
                            <div class="spec-item">
                                <span class="spec-label">외장 색상</span>
                                <strong>{selectedItem.color}</strong>
                            </div>
                        {/if}
                        {#if selectedItem.interior}
                            <div class="spec-item">
                                <span class="spec-label">인테리어</span>
                                <strong>{selectedItem.interior}</strong>
                            </div>
                        {/if}
                        <div class="spec-item">
    ...
    {:else}
        {#if ownedItems.length === 0}
            <div class="empty-state">
                ...
                <p class="empty-subtitle">마켓에서 차량을 구매해보세요.</p>
                <button class="btn-primary" onclick={() => (activeTab = 'market')}>차량 마켓으로</button>
            </div>
...
<style>
    ...
    .back-btn { background: none; border: none; color: var(--color-theme-1); font-size: 0.95rem; font-weight: 600; cursor: pointer; margin-bottom: 1.5rem; padding: 0; }
