<script lang="ts">
  ...
  function getErrorMsg(e: unknown): string {
    if (e instanceof Error) {
      ...
      const msg = e.message;
      if (msg.includes('Insufficient balance') || msg.includes('insufficient')) return '잔액이 부족합니다.';
      if (msg.includes('level') || msg.includes('Level')) return '레벨이 부족합니다.';
      return msg;
    }
    return '계약 체결에 실패했습니다.';
  }
  ...
  async function submitContract() {
    ...
    try {
      ...
      resetFactoryBuild();
      toast.success('계약이 체결되었습니다! 공장 건설을 시작합니다.');
    ...
    <div class="paper-card">
      ...
      <div class="article">
        ...
        <div class="estimate-info">
          <div class="est-row">
            <span>예상 월 생산량</span>
            <span class="est-val">{(estimate.monthlyProduction ?? 0).toLocaleString()}</span>
          </div>
          <div class="est-row">
            <span>직원 수</span>
            <span class="est-val">{(estimate.employeeCount ?? 0).toLocaleString()}명</span>
          </div>
          <div class="est-row">
            <span>창고 용량</span>
            <span class="est-val">{(estimate.warehouseCapacity ?? 0).toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
