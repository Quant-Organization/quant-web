<script lang="ts">
	...
	async function submitBid() {
		...
		if (!amount || amount < selectedAuction.minimumBid) {
			bidError = `최소 입찰가는 ${selectedAuction.minimumBid.toLocaleString()}원입니다.`;
			return;
		}
		bidError = '';
		try {
	...
	async function handlePay(winId: number) {
		...
		try {
			...
			payResults = { ...payResults, [winId]: { error: e instanceof Error ? e.message : '결제 실패' } };
		} finally {
			payingId = null;
		}
	}
...
<div class="container">
	<div class="content">
		...
		{#if loading}
			<div class="loading">경매 데이터를 불러오는 중...</div>
		{:else}
			...
			<div class="auction-details">
				...
				<div class="info-grid">
					<div class="info-box">
						...
						<p class="info-value price-yellow">{selectedAuction.currentHighestBid.toLocaleString()}원</p>
					</div>
					<div class="info-box">
				...
				<div class="bid-section">
					<div class="bid-card">
						...
						<input
								type="text"
								placeholder="입찰 희망 금액 (최소 {selectedAuction.minimumBid.toLocaleString()}원)"
								class="bid-input"
						...
						<div>
							{#each currentBids.slice(0, 5) as bid, index}
								<div class="bid-item">
									...
									<span class="bid-amount">{bid.bidAmount.toLocaleString()}원</span>
								</div>
							{/each}
				...
				{#each activeAuctions as auction}
					<div class="auction-card" onclick={() => openDetailModal(auction)} role="button" tabindex="0"
						...
						<div class="auction-info">
							...
							<p class="auction-price">현재가: {auction.currentHighestBid.toLocaleString()}원</p>
						</div>
					</div>
				{/each}
				{#if activeAuctions.length === 0}
				...
				{#each scheduledAuctions as auction}
					<div class="auction-card" role="button" tabindex="0">
						...
						<div class="auction-info">
							...
							<p class="auction-price">시작가: {auction.startPrice.toLocaleString()}원</p>
						</div>
					</div>
				{/each}
			</div>
			...
			{#if activeTab === '진행 중'}
				{#each myBids as bid}
					<div class="table-row">
						...
						<div>{bid.bidAmount.toLocaleString()}원</div>
			...
			{:else}
				...
				{#each myWins as win}
					<div class="table-row wins-cols">
						...
						<div>{win.winningBidAmount.toLocaleString()}원</div>
				...
				{:else if detailAuction}
					<div class="modal-body">
						...
						<div class="modal-info-row">
							<span>현재 최고 입찰가: <strong class="price-yellow">{detailAuction.currentHighestBid.toLocaleString()}원</strong></span>
